...
export const PokemonType = ({ type }: { type: Type }) => {
  return (
    <span
      key={type.id}
      tw="dark:text-white text-gray-900 ring-gray-4 inline-flex items-center gap-x-1.5 rounded-md px-2 py-1 text-xs font-medium capitalize ring-1 ring-inset"
    >
  ...
  return (
    <div tw="flex items-center text-sm">
      <dt tw="flex flex-1 items-center">
        <p tw="text-gray-900 dark:text-white w-[110px] font-medium">{StatisticMap[statistic].label}</p>
        <div aria-hidden="true" tw="ml-1 flex flex-1 items-center">
          <div tw="relative ml-3 flex-1">
      ...
      <dd tw="text-gray-900 dark:text-white ml-1 w-10 text-right text-sm tabular-nums">{value}</dd>
    </div>
  );
};
        ...
        (
          <div tw="flex w-full flex-col gap-4">
            <div tw="flex flex-row gap-4">
              <Image
                tw="bg-gray-300 dark:bg-gray-4 h-12 w-12 flex-none rounded-full"
                src={pokemon.sprite}
                height={48}
                width={48}
                alt=""
              ...
              <div tw="min-w-0 flex-auto">
                <div tw="flex items-center gap-x-2 leading-5">
                  <p tw="dark:text-white text-gray-900 text-sm font-semibold capitalize leading-6">{pokemon.name}</p>
                  <svg viewBox="0 0 2 2" className="h-0.5 w-0.5 fill-current">
                    <circle cx={1} cy={1} r={1} />
                  </svg>
                  <p tw="text-gray-11 text-xs">{pokemon.weight} kg</p>
                  <svg viewBox="0 0 2 2" className="h-0.5 w-0.5 fill-current">
                    <circle cx={1} cy={1} r={1} />
                  </svg>
                  <p tw="text-gray-11 text-xs">{pokemon.height * 10} cm</p>
                </div>
                <div tw="mt-2 flex flex-wrap gap-x-2">
                  {types.map((type) => {
